...
- [x] 1.3 Add or extend `304 Not Modified` tests to assert no `Content-Length` is invented and the body is empty.
...
- [x] 2.3 Force `304 Not Modified` responses to omit `Content-Length` and send no body even if the dispatch result carries bytes.
